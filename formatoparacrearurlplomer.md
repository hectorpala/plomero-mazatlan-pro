# Formato obligatorio para crear nuevas URLs (Plomero Mazatlán Pro)
...
- Su propósito es preservar: identidad visual (Inter/Montserrat), promesa de llegada 30‑60 min, cobertura por colonias de Mazatlán, datos de contacto visibles y performance equivalente al homepage.
...
3. **Voz**: urgente, profesional, centrada en Mazatlán. Debe mencionar colonias específicas (Las Quintas, Tres Ríos, Chapultepec, Centro, etc.) y tiempos de llegada.
...
**TODAS las páginas deben tener el MISMO nav + header de la homepage** (`https://plomeromazatlanpro.mx/`).
...
- `<title>` = `Servicio + en Mazatlán | Beneficio directo`. Ej.: “Destape de drenajes en Mazatlán | Llegada 30‑60 min”.
- `<meta name="description">` con urgencia + cobertura + contacto (tel/WhatsApp).
- `lang="es-MX"` en `<html>`.
- `<link rel="canonical">` hacia la URL final (sin parámetros).
- OG/Twitter replican título, descripción e imagen hero (`https://plomeromazatlanpro.mx/assets/...`).
- Preloads exactos:  
  ```html
...
4. `Service` específico (nombre = servicio, `serviceType`, `areaServed` Mazatlán, `provider` apuntando al negocio)
...
- `<section id="inicio" class="hero">` con estructura:
  - `h1` con keyword exacta + promesa (“Plomero 24/7 en Mazatlán – Emergencias en 30‑60 min”).
  ...
  - Imagen hero:
    ...
    <picture>
      ...
      <img src="/assets/images/...-800w.webp"
           ...
           alt="Descripción acción + servicio + Mazatlán">
    </picture>
    ```
...
| Elemento | Requisitos |
| --- | --- |
| Hero | WebP 1200×800, `fetchpriority="high"`, `loading="eager"`, `alt` = “Plomero … en Mazatlán …” |
| Cards | WebP 420/800, `width`/`height`, `loading="lazy"`, `decoding="async"` |
| OG/Twitter | Ruta absoluta `https://plomeromazatlanpro.mx/assets/images/...-800w.webp` |
| Renombrado | Usa nombres semánticos (`plomero-destape-drenaje-mazatlan.webp`). |
| Alt text | Formato: `Acción + servicio + ubicación`, ej. “Plomero destapando drenaje en Tres Ríos Mazatlán con equipo rotativo”. |
...
## 5. Copywriting y SEO
1. **Keywords principales**: “plomero en Mazatlán”, “servicio + Mazatlán”, “plomería 24/7 Mazatlán”.
...
4. **Enlaces internos**: usar anchors descriptivos (“Ver precios completos de plomería en Mazatlán”) hacia otras landings/blog.
...
5. [ ] Revisé ortografía y acentos (“plomería”, “Mazatlán”).
