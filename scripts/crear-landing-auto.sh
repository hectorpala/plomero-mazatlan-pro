...
cat > "servicios/${SLUG}/config.json" <<EOF
{
  ...
  "seo": {
    "title": "${SERVICE_NAME} Mazatlán | Servicio Profesional",
    "description": "🔧 ${SERVICE_NAME} en Mazatlán. Servicio profesional certificado. Cotización gratis. ¡Llama: 669 132 5300!",
    "keywords": "${SLUG} mazatlan, plomero mazatlan, servicio plomeria"
  },
  "content": {
    "h1": "${SERVICE_NAME} en Mazatlán | Profesional Certificado",
    "subtitle": "Servicio profesional de ${SERVICE_NAME} en Mazatlán. Atención rápida, garantía por escrito.",
    "whatsapp_text": "${SERVICE_NAME}",
    "breadcrumb": "${SERVICE_NAME}",
    "service_type": "${SERVICE_NAME}"
  },
  "images": {
    "hero_base": "${SLUG}-mazatlan",
    "og": "${SLUG}-mazatlan-800w.webp"
  }
}
EOF
...
if [ -f "scripts/validar-imagenes.sh" ]; then
    ...
    echo "      - ${SLUG}-mazatlan-800w.webp"
    echo "      - ${SLUG}-mazatlan-1200w.webp"
fi
