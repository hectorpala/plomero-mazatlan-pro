...
colonias_modificadas = [
    ...
    "las-palmas", "nuevo-mazatlan", "hacienda-del-valle", "hacienda-los-huertos",
...
for url_elem in root.findall('sm:url', ns):
    ...
    if loc_elem is not None:
        ...
        for colonia_slug in colonias_modificadas:
            if f"/servicios/plomero-colonias-mazatlan/{colonia_slug}/" in loc_text:
                lastmod_elem = url_elem.find('sm:lastmod', ns)
                if lastmod_elem is not None:
                    lastmod_elem.text = new_lastmod
                    updated_count += 1
